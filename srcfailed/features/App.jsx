...
import { buildNotifications, NotificationPanel } from "./notifications/NotificationPanel";
...
export function App() {
  ...
  const [patients, setPatients] = useState([]); // caretaker's patient list
  ...
  useEffect(() => {
    ...
    if (!col) return;
    const onScroll = e => {
      // Find the actual scrolling element inside main-col
      const target = e.target;
      const y = target.scrollTop ?? 0;
      const delta = y - lastScrollY.current;
      if (Math.abs(delta) < 4) return; // ignore tiny movements
      setNavHidden(delta > 0 && y > 60); // hide when scrolling down past 60px
  ...
  const NAV = [
    { id: "home", label: "Home", NavIc: Icons.navHome },
    { id: "medicines", label: "Medicines", NavIc: Icons.navMeds },
    { id: "profile", label: "Profile", NavIc: Icons.navProfile },
  ];
          ...
          <div className="app">
            ...
            <div className="app-inner">
              ...
              <div className="main-col" ref={mainColRef}>
                ...
                {/* Header — teal on tablet/desktop, white on mobile */}
                <div className="hdr">
                  {/* ── Caretaker viewing banner — shows on ALL screen sizes ── */}
                  {activePatient && (
                    <div style={{
                  ...
                  <div className="hdr-row">
                    ...
                    <div className="hdr-actions">
                      ...
                      <div style={{ position: "relative" }}>
                        <button className="hdr-ic-btn" onClick={() => { setNotifOpen(o => !o); setFabOpen(false); }}>
                          <Icons.bell />
                          {(() => {
                ...
                {(() => {
                  ...
                  return (<>
                    {tab === "home" && <Dashboard key={refresh} userId={eId} onMeds={() => setTab("medicines")} onRx={() => setSlide("rx")} onExpiry={() => setSlide("expiry")} onReports={() => setSlide("reports")} />}
                    {tab === "medicines" && <MedsTab userId={eId} toast={showToast} caretakerPatientId={isCtk ? eId : null} />}
                    {tab === "profile" && <ProfileTab user={user} userId={user.id} onLogout={handleLogout} onOpenRx={() => setSlide("rx")} onSwitchPatient={p => { setActivePatient(p); setTab("home"); }} activePatient={activePatient} />}
              ...
              <nav className={`bnav${navHidden ? " nav-hidden" : ""}`}>
                ...
                {NAV.filter(n => n.id !== "profile").map(({ id, label, NavIc }) => (
                  <button key={id}
                    className={`nit${tab === id ? " on" : ""}`}
                    onClick={() => { setTab(id); setSlide(null); setSearchOpen(false); setNavHidden(false); lastScrollY.current = 0; if (id !== "home") setFabOpen(false); }}>
                    <NavIc active={tab === id} />
                    <span className="nit-label">{label}</span>
                  </button>
                ))}
                ...
                {NAV.filter(n => n.id === "profile").map(({ id, label, NavIc }) => (
                  <button key={id}
                    className={`nit nit-profile${tab === id ? " on" : ""}`}
                    onClick={() => { setTab(id); setSlide(null); setSearchOpen(false); setNavHidden(false); lastScrollY.current = 0; setFabOpen(false); }}>
                    <NavIc active={tab === id} />
                    <span className="nit-label">{label}</span>
                  </button>
                ))}
