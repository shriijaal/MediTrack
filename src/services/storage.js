...
// ── Auth / Session ──────────────────────────────────────────────
export const getUsers    = ()    => safeParse("mt_users", []);
export const saveUsers   = u     => localStorage.setItem("mt_users", JSON.stringify(u));
export const getSession  = ()    => safeParse("mt_sess", null);
...
// ── Medicines ───────────────────────────────────────────────────
export const getMeds  = id    => safeParse("mt_meds" + id, []);
export const saveMeds = (id, m) => localStorage.setItem("mt_meds" + id, JSON.stringify(m));

// ── Prescriptions ───────────────────────────────────────────────
export const getRx    = id    => safeParse("mt_rx" + id, []);
...
export const getDoseLog  = (userId, date) =>
  safeParse(`mt_doses_${userId}_${date}`, {});
...
export const getSeenNotifs = (userId) =>
  safeParse(`mt_seen_notifs_${userId}`, []);
...
export const getFiredNotifs = (userId) =>
  safeParse(`mt_notif_fired_${userId}`, {});
